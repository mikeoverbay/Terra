...
Public Class frmServer
	...
	Dim connect_buffer As New List(Of String)
				...
				Select Case inc.MessageType
					...
					Case NetIncomingMessageType.ConnectionApproval
						If inc.ReadByte() = CByte(packetType.login) Then ' code for login
							...
							'Write_chat(client.Name + " is connecting.." + vbCrLf)
							If client.client_type = clientType.host Then
								For Each c In Serverclients
									If c.client_type = clientType.host Then
										client.connection.Disconnect(c.Name + " is already hosting this server!" + vbCrLf + "Join the session instead.")
										write_diagnostics(client.Name + " is trying to connect as host but (" + c.Name + ") is already hosting" + vbCrLf)
										Exit Select
									End If
								Next
							End If
							Write_connect(client.Name + " Connected" + vbCrLf)
							...
							For Each c In Serverclients
								'this client just connected and 
							...
							Next
							Server.SendMessage(outMsg, Server.Connections, NetDeliveryMethod.ReliableOrdered, 0)
							Write_connect("Login Ack: " + client.Name + vbCrLf)
...
time_:
		...
		End While
	End Sub
	Private Sub write_chat_buffer(st As String)
		Try
	...
	Private Sub Write_connect(ByVal st As String)
		If Me.InvokeRequired Then
			Try
				Me.Invoke(New AppendChat(AddressOf Write_connect), New Object() {st})
			Catch ex As Exception
			End Try

		Else
			Try
				If connect_buffer.Count > 30 Then
					connect_buffer.RemoveAt(0)
				End If
				connect_buffer.Add(st)
				chat_text_tb.Clear()
				Dim s As String = ""
				For Each m In connect_buffer
					s += m
				Next
