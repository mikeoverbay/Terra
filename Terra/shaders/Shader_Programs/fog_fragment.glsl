...
void main(void)
{
  ...
  vec2 tv1;
    ...
    if (enable_fog == 1) {
      ...
      float  light_distance = distance(v_pos.xyz,lightPos.xyz);
      
      if (light_distance<800.0)
{
    light_distance = 1.0 - light_distance/800.0;
    light_distance = tan(light_distance*1.570796327);
    }
    else
    {
    light_distance = 0.0;
...
//gl_FragColor.r += light_distance;
        float ni = texture2D(noiseMap,fog_uv_coords*2.0).r*-0.5;
        vec4 fog_c = gl_Fog.color+ (vec4(1.0, 1.0, 0.4, 0.0)*light_distance*.1);
        float dd = tan(1.0-(Vposition.y/y_pos)+ni) * gl_Fog.density * 2.0;
        float ff = -dd * dd * z * z * LOG2;
        float fd = exp2(ff * 0.1 + ((1.0-light_distance)*.01));
        gl_FragColor = mix(fog_c, gl_FragColor, clamp(fd, 0.0, 1.0));
    }
 else {
        gl_FragColor = gl_FragColor;
    }
    //gl_FragColor.rg += fog_uv_coords.xy;
}
