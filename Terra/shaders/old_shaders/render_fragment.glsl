...
void main(void)
{
  ...
  diffuse += ((Coutmix*diffuse) *2.0);

  base = base + (t4 * MixLevel.a) ;
  base = base + (t3 * MixLevel.b) ;
  base = base + (t2 * MixLevel.g) ;
  base = base + (t1 * MixLevel.r) ;

 
  // final color mix

  gl_FragColor.rgb = diffuse + (base.rgb* l_ambient) ;
